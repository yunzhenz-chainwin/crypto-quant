...
**一句話**：一台機器、一個 uvicorn 進程、前後端同吃區網位址 `10.201.7.12:8000`；由 Task Scheduler ＋看門狗保證開機自啟與崩潰自癒。本機開發另用 API 8001，避免與 TalentHub 的 loopback 8000 衝突。
...
# 清掉殘留的 Crypto Quant 10.201.7.12:8000 進程（勿終止 TalentHub 的 127.0.0.1:8000）
...
| 對外公開 | 開 Cloudflare Quick Tunnel 指向 `10.201.7.12:8000`（網址每次會變 → 根治見待辦 #64 named tunnel） |
...
| 區網 8000 被佔、起不來 | Crypto Quant 殘留進程 | 只清除綁定 `10.201.7.12:8000` 的程序；勿終止 TalentHub 的 `127.0.0.1:8000` |
...
| 後端顯示 `startup refused` | 對外模式仍是 fallback，或本機 fallback 沒有明示 loopback/override | 正式環境確認 `secrets.local.cmd` 已由 `start_backend.cmd` 載入；本機開發依開發指南設定三個安全環境變數 |
