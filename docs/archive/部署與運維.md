...
> 最後更新：2026-07-08 · 搭配 [README](../../README.md)、[開發指南](開發指南.md) 一起看。
...
flowchart TD
    ...
    CMD -.->|"載入"| SEC["secrets.local.cmd<br/>ADMIN/OPENAI 環境變數"]
...
| 密鑰 | `secrets.local.cmd`（`set` 環境變數） | 專案根、gitignored |
...
> GPT 金鑰也可改在**後台「AI 設定」頁**填（存 `app_config`）；環境變數 `OPENAI_API_KEY` 優先。
...
set "OPENAI_API_KEY=<sk-... 選填,不填 AI 走規則引擎>"
```
