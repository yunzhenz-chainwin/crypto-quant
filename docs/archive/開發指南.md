...
> 部署與運維另見 [部署與運維](部署與運維.md)。最後更新：2026-07-13
...
- 後台 `/admin` 讀 `ADMIN_USER` / `ADMIN_PASS`。正式與排程啟動應先載入 `secrets.local.cmd`；若未載入會回到本機預設 `admin` / `admin123` 並印安全警告。
...
| `python src\verify_indicators.py [1d\|1h]` | **指標交叉驗證**：用獨立演算法逐點重算比對；後台 `/admin/verify/indicators` 同源 | 改指標計算後 |
