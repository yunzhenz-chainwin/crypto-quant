...
**每日 09:00 排程(`scheduler.run_pipeline`；2026-07-06 起，日線 UTC 收盤後 1 小時)自動做**:抓 K 線 → 算指標 → 算相關性 → 入庫 `prices`/`indicators` → 重算 `daily_signal` → 更新 `fear_greed`,並把這次執行記進 `job_runs`。
